...
- A `ValueError` raised in `services/` or `markdown/` reaches the calling LLM verbatim — `logged_tool` (`src/kajet_turbo/log.py`) maps it straight to `ToolError`. Write those messages in English and name the parameter at fault.

...
from tests.services.conftest import build_note_wiring, note_target, workspace_target
...
class _RecordingIndexer:
    ...
    def index_many(self, workspace, owner_id, notes):
        pass


def test_save_triggers_indexing(database, git_workspace_factory):
    indexer = _RecordingIndexer()
    service = build_note_wiring(database, indexer=indexer)
    ws = git_workspace_factory("ws")
    service.create.save(workspace_target("u1", "ws", ws), "Title", "# Title\n\nbody\n", tags=[])
...
def test_save_writes_fts_via_indexer(database, git_workspace_factory):
    ...
    ws = git_workspace_factory("ws")
    service.create.save(
        workspace_target("u1", "ws", ws), "Title", "# Title\n\nsearchable body\n", tags=[]
    )
...
def test_save_surfaces_chunk_write_failure(database, git_workspace_factory):
    ...
    service = build_note_wiring(database, indexer=_BadIndexer())
    ws = git_workspace_factory("ws")
...
def test_delete_does_not_require_indexer_cleanup(database, git_workspace_factory):
    ...
    service.delete.delete(note_target("u1", "ws", ws, res["note_id"]))
    assert service.crud_repo.get(res["note_id"], owner_id="u1") is None


def test_no_indexer_is_a_noop(database, git_workspace_factory):
    service = build_note_wiring(database)  # no indexer
    ws = git_workspace_factory("ws")
    res = service.create.save(
        workspace_target("u1", "ws", ws), "Title", "# Title\n\nbody\n", tags=[]
    )

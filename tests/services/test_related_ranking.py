...
def _ev(source_rowid, target_note_id, chunk_id, distance):
...
def test_rank_related_prefers_closer_target():
    ...
    assert ranked[0].source_rowid == 1
...
def test_rank_related_tail_always_ranks_after_head():
    ...
    evidence = [_ev(1, note_id, f"c-{note_id}", dist) for note_id, dist in targets]

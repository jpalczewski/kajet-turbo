...
def test_export_folder_concatenates_notes_with_headings(service, workspace):
    service.save("u1", "ws", str(workspace), "01 First", "first body", [], folder="a")
    service.save("u1", "ws", str(workspace), "02 Second", "second body", [], folder="a")
...
def test_export_folder_includes_subtree(service, workspace):
    service.save("u1", "ws", str(workspace), "Nested", "nested body", [], folder="a/b")
    result = service.export_folder("ws", "u1", str(workspace), "a")
...
def test_export_folder_excludes_sibling_folders(service, workspace):
    service.save("u1", "ws", str(workspace), "In", "in body", [], folder="a")
    service.save("u1", "ws", str(workspace), "Out", "out body", [], folder="b")
    result = service.export_folder("ws", "u1", str(workspace), "a")
    assert result["note_count"] == 1
...
def test_export_folder_truncates_at_note_boundary(service, workspace):
    service.save("u1", "ws", str(workspace), "01 First", "x" * 50, [], folder="a")
    service.save("u1", "ws", str(workspace), "02 Second", "y" * 50, [], folder="a")
...
def test_export_folder_always_includes_first_note_even_if_oversized(service, workspace):
    service.save("u1", "ws", str(workspace), "01 Huge", "z" * 200, [], folder="a")

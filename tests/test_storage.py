...
def test_delete_note(notes):
    notes.insert("abc1234", "ws1", "Do usunięcia", [], _now(), _now(), "treść")
    notes.delete("abc1234")
    assert notes.get("abc1234") is None


def test_list_notes_by_workspace(notes):
    notes.insert("id1", "ws1", "Notatka 1", ["a"], _now(), _now(), "treść 1")
    notes.insert("id2", "ws1", "Notatka 2", ["b"], _now(), _now(), "treść 2")
    notes.insert("id3", "ws2", "Notatka 3", [], _now(), _now(), "treść 3")
    result = notes.list("ws1")
...
def test_list_notes_filter_by_tag(notes):
    notes.insert("id1", "ws1", "Tagged", ["python", "mcp"], _now(), _now(), "treść")
    notes.insert("id2", "ws1", "Untagged", [], _now(), _now(), "treść")
    result = notes.list("ws1", tags=["python"])
    ids = [n["id"] for n in result]
    assert "id1" in ids
...
def test_fts_search_finds_by_title(notes):
    notes.insert("id1", "ws1", "Python async programming", ["python"], _now(), _now(), "tutorial o asyncio")
    notes.insert("id2", "ws1", "JavaScript basics", ["js"], _now(), _now(), "podstawy JS")
    results = notes.search_fts("async", "ws1")
...
def test_fts_search_finds_by_content(notes):
    notes.insert("id1", "ws1", "Notatka", [], _now(), _now(), "sqlite jest świetny do embeddingów")
    results = notes.search_fts("embedding", "ws1")
    assert any(r["id"] == "id1" for r in results)


def test_fts_search_respects_workspace(notes):
    notes.insert("id1", "ws1", "Python notatka", [], _now(), _now(), "treść")
    notes.insert("id2", "ws2", "Python inny workspace", [], _now(), _now(), "treść")
    results = notes.search_fts("Python", "ws1")
    ids = [r["id"] for r in results]
    assert "id1" in ids
    assert "id2" not in ids
...
def test_fts_search_trigram_partial(notes):
    notes.insert("id1", "ws1", "Programowanie", [], _now(), _now(), "nauka programowania w Pythonie")
    results = notes.search_fts("gram", "ws1")
    assert any(r["id"] == "id1" for r in results)


def test_update_note_title_only_preserves_fts_content(notes):
    notes.insert("abc1234", "ws1", "Stary tytuł", [], _now(), _now(), "unikalna treść notatki")
    notes.update("abc1234", title="Nowy tytuł", updated_at=_now())
    results = notes.search_fts("unikalna", "ws1")
...
def test_insert_vec_and_search(db_small_dim):
    ...
    repo.insert("id1", "ws1", "Notatka A", [], _now(), _now(), "treść A")
    repo.insert("id2", "ws1", "Notatka B", [], _now(), _now(), "treść B")
    ...
    results = repo.search_vec(_fake_embedding(0.1), "ws1", k=2)
    assert results[0]["id"] == "id1"


def test_hybrid_search_fallback_without_vec(notes):
    notes.insert("id1", "ws1", "Python tutorial", [], _now(), _now(), "programowanie w Pythonie")
    results = notes.hybrid_search("Python", "ws1")
    assert any(r["id"] == "id1" for r in results)


def test_hybrid_search_with_vec(db_small_dim):
    repo = NoteRepository(db_small_dim.engine)
    repo.insert("id1", "ws1", "Notatka wektorowa", [], _now(), _now(), "treść z wektorem")
    ...
    results = repo.hybrid_search("wektorowa", "ws1", embedding=_fake_embedding(0.5))
    assert any(r["id"] == "id1" for r in results)

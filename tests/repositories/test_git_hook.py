...
def test_commit_fires_post_commit_hook(tmp_path):
    calls: list[str] = []
    hooks = PostCommitHooks()
    hooks.register(calls.append)

    ws = tmp_path / "ws"
    GitRepository.init(str(ws))
    (ws / "n.md").write_text("x")
    GitRepository(str(ws), hooks).commit_file("n.md", "note: add")

    assert calls == [str(ws)]


def test_hook_exception_does_not_break_commit(tmp_path):
    hooks = PostCommitHooks()
    hooks.register(lambda _p: (_ for _ in ()).throw(RuntimeError("boom")))
...
def test_transaction_coalesces_hooks_and_fires_after_release(tmp_path):
    calls: list[str] = []
    hooks = PostCommitHooks()
    hooks.register(calls.append)

    ws = tmp_path / "ws"
    repo = GitRepository.init(str(ws), hooks)
...
def test_transaction_fires_hook_before_deferred_failure(tmp_path):
    calls: list[str] = []
    hooks = PostCommitHooks()
    hooks.register(calls.append)

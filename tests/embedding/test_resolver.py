...
def _user(database, uid="u1"):
    with Session(database.engine) as s:
        s.add(User(id=uid, email=f"{uid}@e.com", created_at="2026-01-01")); s.commit()
...
def test_resolves_active_profile(database):
    ...
    repo.create("u1", "P", "https://api.openai.com/v1", "text-embedding-3-large",
                _CIPHER.encrypt("sk-user"), 3072)
...
def test_keyless_profile_yields_none_key(database):
    _user(database)
    EmbeddingProfileRepository(database.engine).create("u1", "local", "http://local/v1", "m", None, 8)
    cfg = _resolver(database).resolve_backend("u1")

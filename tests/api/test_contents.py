...
def test_contents_root_returns_folders_and_notes(auth_client):
    ...
    note_service.create.save(_ws(workspace), "Root Note", "content", [])
    note_service.create.save(_ws(workspace), "Deep Note", "content", [], folder="docs")
...
def test_contents_subfolder_returns_folder_notes(auth_client):
    client, note_service, workspace = auth_client
    note_service.create.save(_ws(workspace), "Doc", "content", [], folder="docs")
    note_service.create.save(_ws(workspace), "Root", "content", [])
...
def test_contents_note_path_resolves_without_404(auth_client):
    client, note_service, workspace = auth_client
    note_id = note_service.create.save(_ws(workspace), "Doc", "content", [], folder="docs")[
        "note_id"
    ]
...
def test_contents_sets_default_note_for_readme(auth_client):
    client, note_service, workspace = auth_client
    readme_id = note_service.create.save(_ws(workspace), "README", "intro", [], folder="docs")[
        "note_id"
    ]
    note_service.create.save(_ws(workspace), "Other", "content", [], folder="docs")
